...
Firstly, we had to implement a sequential version of the Game of Life. We created a class \texttt{Grid} that represents the game board. The grid is a 2D array of cells, where each cell can be either alive or dead. The class provides methods to set and get the state of a cell, as well as to count the number of alive neighbors of a cell. Interios, we used an array of unsigned chars to store the state of the cells. This way, we can store the state of 8 cells in a single byte. This is important for the parallel version, as we can send the state of multiple cells in a single message. In this single threaded sequential version we have to ensure that the game loops back around at the edges of the grid. This is done by using the modulo operator to calculate the index of the cell to the left, right, above or below the current cell. All the other implementation details can be found in the code.
